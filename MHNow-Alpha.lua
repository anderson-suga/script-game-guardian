...
function mainMenu()
  firstMenu = gg.choice({"[1]-SpeedNoteRemove", "[2]-HunterOptions", "[3]-EnemyOptions", "[4]-MapOptions", "[5]-Full GodMode", "[6]-SpeedHack", "[7]-MultiChoice", "[QUICK-OPTIONS]","❌EXIT❌","👉INFOS👈"}, nil, "Made by Hylianer")
...
function cheat_2()
			...
			-- Meldung anzeigen
			local menuPL = gg.choice({'Change Attack','Change ElementAttack','Change Defense','Change Crit.-Damage%','SpeedUpHunter','Change WeaponElement','PrivacyMode','SetTPWeaponLevels','SetTPArmorLevels','SuperHealth','SuperRecovery'},nil,'Select Option')
			gg.setVisible(false)
			...
			elseif menuPL == 1 then
						...
						local menuAT = gg.choice({'2000','8000','GODMODE','Own Value'},nil,'Select Value')			
						gg.getResults(9000)
						-- Wenn der Benutzer "Abbrechen" klickt,
						if menuAT == 0 then
						-- Keine Aktion durchführen
			...
			elseif menuPL == 3 then
						...
						local menuAT = gg.choice({'5000','10000','GODMODE','Own Value'},nil,'Select Value')			
						gg.getResults(9000)
						-- Wenn der Benutzer "Abbrechen" klickt,
						if menuAT == 0 then
						-- Keine Aktion durchführen
...
function cheat_3()
		...
		-- Meldung anzeigen
			local menuPL = gg.choice({'EasyPartBreaker','FreezeNearMonster','SmallMonsterInstaKill','LastHitIsNotSlomo'},nil,'Select Option')
			gg.setVisible(false)
			-- Wenn der Benutzer "Abbrechen" klickt,
			if menuPL == 0 then
				-- Keine Aktion durchführen
			...
			elseif menuPL == 3 then
					...
					gg.alert('READ! in this mode your attack is set to 0 but your elemental attack is very high. \nWhich means you can only defeat large monsters with elemental weaknes!')
					valueFromClass("WeaponLevelSpecSettingsProto", "0x1C", false, false, gg.TYPE_DWORD)
					gg.getResults(9000)
					gg.editAll(0, gg.TYPE_DWORD)
					gg.clearResults()
